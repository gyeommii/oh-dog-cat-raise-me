...
function domContentLoaded() {
  if (location.href.includes("result=f")) {
    alert("비밀번호 설정 중 실패하였습니다. 다시 시도해주세요.");
  }
...
// 비밀번호 유효성 검사에 필요한 변수들
  ...
  function keyupEventPassword(e) {
    ...
    */
    ...
    if (pwReg.test(password)) {
      ...
      pwHelp.innerHTML = "비밀번호 조건을 다시 확인해주세요! 조건) 8글자 이상의 영어와 숫자 조합";
  ...
  pwBtn.addEventListener("click", async () => {
    const member_id = document.getElementById("input-memberid").value;
    const email = document.getElementById("input-email").value;
    const password = passwordInput.value;

    const data = {member_id, email, password};

    console.log(data);

    const {data: result} = await axios.patch("./password", data);

    if (result) {
      location.href = '../../signin';
    } else {
      location.href = `./password?result=f&email=${email}&member_id=${member_id}`;
    }
  })
}
