            ...
            <div class="margin-class" style="width: 80;">
                <div>
                    <div class="search-container">
                        <c:url var="postSearchPage"
                            value="/community/search" />
                    ...
                    <div class="row">
                        <div class="col-3">
                            ...
                            <select class="form-control"
                                ...
                                <option value="0"
                                    ${currentCategory == '0' ? 'selected' : ''}>전체</option>
                                <option value="1"
                                    ${currentCategory == '1' ? 'selected' : ''}>내새꾸
                                    자랑</option>
                                <option value="2"
                                    ${currentCategory == '2' ? 'selected' : ''}>[입양]키워주개</option>
                                <option value="3"
                                    ${currentCategory == '3' ? 'selected' : ''}>[입양]키워주냥</option>
                                <option value="4"
                                    ${currentCategory == '4' ? 'selected' : ''}>[실종/제보]길잃은멍</option>
                                <option value="5"
                                    ${currentCategory == '5' ? 'selected' : ''}>[실종/제보]길잃은냥</option>
                            </select>
                        </div>
                        <!-- 정렬 버튼 -->
                        <div class="btn-group col-3" role="group"
                            aria-label="Basic example">
                            <button onclick="return sortPosts('desc')"
                                class="btn">최신글</button>
                            <button onclick="return sortPosts('asc')"
                                class="btn">오래된글</button>
                        </div>
                        <div class="col-3"></div>
                        ...
                        <div class="col-3">
                            <a href="../community/createPost">
                                <button
                                    class="btn btn-custom btn-custom-width">글쓰기</button>
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="card mb-3">
                        <div class="card-body">
                            <h5 class="card-title ellipsis">
                                <a href="${postDetailsPage}"> <span
                                    class="category-label"> <c:choose>
                                            <c:when
                                                test="${post.post_category_fk == 1}">내새꾸 자랑</c:when>
                                            <c:when
                                                test="${post.post_category_fk == 2}">[입양]키워주개</c:when>
                                            <c:when
                                                test="${post.post_category_fk == 3}">[입양]키워주냥</c:when>
                                            <c:when
                                                test="${post.post_category_fk == 4}">[실종/제보]길잃은멍</c:when>
                                            <c:when
                                                test="${post.post_category_fk == 5}">[실종/제보]길잃은냥</c:when>
                                        </c:choose>
                                </span> ${post.title}
                                </a>
                            </h5>
